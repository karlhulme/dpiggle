import { assertEquals, assertSpyCalls, assertRejects, spy } from '../deps.ts'
import { retryable, OperationTransitoryError, OperationInterruptedError } from '../src/index.ts'
...
Deno.test('An operation that fails with transitory bespoke errors is re-tried and can succeed.', async () => {
  ...
  const result = await retryable(testOp, { isErrorTransient: err => err instanceof BespokeError })
  assertEquals(result, 123)
...
Deno.test('An operation that fails with transitory errors too many times raises the last transitory error.', async () => {
  ...
  await assertRejects(() => retryable(testOp, { retryIntervalsInMilliseconds: [100, 200] }), OperationTransitoryError, 'LAST')
...
Deno.test('An operation can be interrupted and this is checked before the first invocation.', async () => {
  ...
  await assertRejects(() => retryable(testOp, { canContinueProcessing: () => false }), OperationInterruptedError)
